...
describe("worker", function() {
  ...
  describe("configuration", function() {
    ...
    describe("should extend via [use]", function() {
      ...
      it("with options", function() {
        ...
        // then
        expect(called).to.eql([worker, { foo: "BAR" }]);
      });
  ...
  describe("work subscription", function() {
    ...
    it("should subscribe with local options", function() {
      ...
      // when
      var workerDefinition = worker.subscribe(
        "worker:Stuff",
        {
          lockDuration: 3000,
          variables: ["foo"]
  ...
  describe("task execution", function() {
    ...
    it("should execute process", async function() {
      ...
      var { id } = await engineApi.startProcessByKey(
        "TestProcess",
        startVariables
      );
      ...
      worker.subscribe(
        "work:A",
        ["numberVar", "objectVar", "dateVar", "nonExistingVar"],
      ...
      worker.subscribe(
        "work:B",
        ["booleanVar", "nestedObjectVar", "stringVar"],
        function(context) {
          trace.push(log(context));
      ...
      expect(await engineApi.getProcessInstance(id)).not.to.exist;
    });
    ...
    it("should execute process / no backoff", async function() {
      ...
      // (2) promise based worker
      worker.subscribe("work:B", ["stringVar", "nestedObjectVar"], function(
        context
      ) {
        trace.push(log(context));
    ...
    describe("should handle error", function() {
      ...
      it("promise rejection", async function() {
        ...
        worker.subscribe("work:A", function(context) {
          return Promise.reject(new Error("could not execute"));
        });
    ...
    it("should trigger BPMN error", async function() {
      // given
      const { id } = await engineApi.startProcessByKey("TestProcess");
  ...
  describe("variable serialization", function() {
    it("should preserve Object serialization", async function() {
      ...
      var existingUserVar = await engineApi.getProcessVariable(
        id,
        "existingUser"
      );
